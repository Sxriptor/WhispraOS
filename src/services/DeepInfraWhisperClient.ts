...
export interface DeepInfraWhisperRequest {
  audio: Blob;
  model?: string;
  language?: string;
  prompt?: string;
...
export interface DeepInfraWhisperResponse {
  text: string;
  language?: string;
  duration?: number;
...
export interface DeepInfraSegment {
  id: number;
  seek: number;
  start: number;
  end: number;
...
export interface DeepInfraWhisperConfig {
  baseUrl: string;
  model: string;
  temperature: number;
  maxRetries: number;
...
export class DeepInfraWhisperClient {
  private apiKeyManager: ApiKeyManager;
  private config: DeepInfraWhisperConfig;

  constructor(apiKeyManager: ApiKeyManager, config: Partial<DeepInfraWhisperConfig> = {}) {
  ...
  async transcribe(request: DeepInfraWhisperRequest): Promise<DeepInfraWhisperResponse> {
    ...
    for (let attempt = 1; attempt <= this.config.maxRetries; attempt++) {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Deep Infra Whisper API error (${response.status}): ${errorData.error?.message || response.statusText}`
          );
        }
        ...
        if (errorMessage.includes('401') || errorMessage.includes('403')) {
          throw new Error('Invalid Deep Infra API key or insufficient permissions');
        }
    ...
    const finalError = lastError || new Error('Deep Infra transcription failed after all retry attempts');
    ErrorReportingService.getInstance().captureError(finalError, {
      category: ErrorCategory.API,
      severity: ErrorSeverity.HIGH,
      component: 'DeepInfraWhisperClient',
  ...
  private createTestAudioBlob(): Blob {
    // Create a minimal WAV file with silence (44 bytes header + 100ms of silence at 16kHz)
  ...
  updateConfig(newConfig: Partial<DeepInfraWhisperConfig>): void {
    this.config = { ...this.config, ...newConfig };
  }

  getConfig(): DeepInfraWhisperConfig {
    return { ...this.config };
  }
